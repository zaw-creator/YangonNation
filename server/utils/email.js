...
const FROM = process.env.RESEND_FROM || 'Yangon Nation <noreply@yangonnation.com>';

async function sendInviteEmail({ to, name, token }) {
  const link    = `${process.env.CLIENT_URL}/invite/${token}`;
  const expires = new Date(Date.now() + 14 * 24 * 60 * 60 * 1000).toLocaleDateString('en-GB', {
  ...
  await resend.emails.send({
    ...
    html: `
      <div style="background:#1A1A1A;padding:48px;font-family:sans-serif;color:#fff;max-width:600px;margin:0 auto;">
        ...
        <h1 style="font-size:36px;font-weight:900;letter-spacing:0.06em;color:#fff;margin:8px 0 0;">
          YANGON <span style="color:#C8A84B;">NATION</span>
        </h1>
        <hr style="border:none;border-top:1px solid #333;margin:28px 0;">
        <h2 style="color:#fff;font-size:22px;margin:0 0 12px;">Welcome back, ${name}.</h2>
        <p style="color:rgba(255,255,255,0.55);line-height:1.75;margin:0 0 28px;">
        ...
        <a href="${link}"
          style="display:inline-block;padding:14px 32px;background:#C8A84B;color:#1A1A1A;font-weight:700;font-size:12px;letter-spacing:0.12em;text-transform:uppercase;text-decoration:none;border-radius:3px;">
          Re-register Now →
        </a>
        <p style="color:rgba(255,255,255,0.25);font-size:11px;margin-top:24px;">
          Link expires: ${expires}. Single-use only.
        </p>
        <hr style="border:none;border-top:1px solid #2a2a2a;margin:28px 0;">
        <p style="color:#444;font-size:10px;letter-spacing:0.15em;text-align:center;margin:0;">
  ...
  await resend.emails.send({
    ...
    html: `
      <div style="background:#1A1A1A;padding:48px;font-family:sans-serif;color:#fff;max-width:600px;margin:0 auto;">
        ...
        <hr style="border:none;border-top:1px solid #333;margin:28px 0;">
        <h2 style="color:#fff;font-size:22px;margin:0 0 12px;">Thank you, ${name}.</h2>
        <p style="color:rgba(255,255,255,0.55);line-height:1.75;margin:0;">
